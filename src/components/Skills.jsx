...
import { PiBrainDuotone, PiCloudDuotone, PiChartBarDuotone, PiShieldCheckDuotone, PiCodeDuotone, PiRocketLaunchDuotone } from 'react-icons/pi'
...
export default function Skills() {
  return (
    <section id="skills" className="relative py-16 lg:py-20 bg-surface-100/50">
          ...
          >
            {expertAreas.map((area, idx) => (
              ...
              >
                {/* 3D Animated Icon */}
                <div className="mb-4">
                  <AnimatedIcon3D color={area.color} size="md" animation={area.animation} delay={idx * 0.2}>
                    <area.icon className="w-full h-full" />
                  </AnimatedIcon3D>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {area.skills.map((skill, skillIdx) => (
                    <motion.span
                      ...
                      viewport={{ once: true }}
                      transition={{ delay: skillIdx * 0.03, duration: 0.3 }}
                      whileHover={{ scale: 1.08, y: -2 }}
                      className="text-[11px] font-medium px-2.5 py-1 rounded-full bg-surface-50 text-surface-600 border border-surface-200 hover:border-primary-300 hover:text-primary-700 hover:bg-primary-50 transition-colors cursor-default"
                    >
                      {skill}
                    </motion.span>
                  ))}
